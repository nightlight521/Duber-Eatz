...
/* Class Name: Level4Plus
 * Version: 1.0
 * Author: Paula Yuan
 * Date: October 24, 2019
 * Description: Level 4+ Duber Eatz solution
 */
...
class Level4Plus {
  ...
  public static PartialPath findPath(char[][] map, boolean[][] backtrackMap, int row, int column, int deliveries, int delivered, int steps, int tips) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      path.path.add(i);  // Add the direction to the end of the path. The path builds in reverse and will later be read backwards
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int i = solution.path.size()-1; i >= 0; i--) {
      ...
      } else if (mapArray[row][column] != 'S' && mapArray[row][column] != 'X') {
        mapArray[row][column] = 'x';
      }
    }
